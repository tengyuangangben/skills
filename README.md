...
- 当前稳定版本：`v2.0.0`
- 最新发布页：`https://github.com/tengyuangangben/skills/releases/tag/v2.0.0`
- 版本历史：见 `CHANGELOG.md`
...
| 版本 | 说明 |
|---|---|
| v2.0.0 | 配置文件级安全开关（提交人/渠道/确认提交/OCR限制）、删除先查ID再删、update/update_attachment 透传提交人渠道、字段键名兼容增强 |
| v1.1.2 | 新增删除能力（delete模式+删除脚本）、token配置文件优先级、提交渠道运行时优先级修复、附件默认直传（仅用户明确要求才识别） |
...
- 从 `v1.0.0` 升级：先到 `v1.1.0`，再升级到 `v2.0.0`
- 从 `v1.1.0`、`v1.1.1` 或 `v1.1.2` 升级：直接升级到 `v2.0.0`
- 升级后建议：
  - 重新发布 WPS 端 `录入脚本.js`
  - 使用 `setup` 做一次健康检查
